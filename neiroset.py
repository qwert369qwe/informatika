...
class ImprovedNeuralInterface:
    ...
    def _process_output(self, output_vector, original_task):
        """Преобразует выход нейросети в исполняемый код (исправленная версия)"""
        output_vector = output_vector.flatten()
        
        # ----- ПЕРВООЧЕРЕДНОЙ АНАЛИЗ ТЕКСТА ЗАДАЧИ (обход неверной классификации) -----
        task_lower = original_task.lower()
        
        # 1. Распознавание задачи на повторение текста заданное число раз
        #    Пример: "вывести текст «Python is awesome!» 10 раз"
        repeat_patterns = [
            r'вывести\s+текст\s*[«"“](.+?)[»"“]\s*(\d+)\s*раз',
            r'напечатать\s+фразу\s*[«"“](.+?)[»"“]\s*(\d+)\s*раза?',
            r'повторить\s*(\d+)\s*раз\s*[«"“](.+?)[»"“]',
            r'выводит\s+текст\s*[«"“](.+?)[»"“]\s*(\d+)\s*раз',
            r'вывести\s*[«"“](.+?)[»"“]\s*(\d+)\s*раз',
            r'текст\s*[«"“](.+?)[»"“]\s*(\d+)\s*раз',
            r'напечатать\s*(\d+)\s*раз\s*[«"“](.+?)[»"“]',
        ]
        
        for pattern in repeat_patterns:
            match = re.search(pattern, original_task, re.IGNORECASE)
            if match:
                groups = match.groups()
                # Определяем где текст, где число
                if len(groups) == 2:
                    if groups[0].isdigit():
                        count = int(groups[0])
                        text = groups[1]
                        ...
                        # если ни один не число (маловероятно) — берем первое как текст
                        text = groups[0]
                        count = 10
                else:
                    text = groups[0] if groups else "Python is awesome!"
                    count = 10
        ...
        # 2. Если не сработало, используем классификацию по выходу нейросети
        task_type_map = {
        ...
        if task_type == 'fixed_loop':
            # Для fixed_loop стараемся извлечь текст и количество из задачи
            # Сначала ищем фразу в кавычках
            quote_match = re.search(r'[«"“](.+?)[»"“]', original_task)
            if quote_match:
                text = quote_match.group(1)
            else:
                # Если кавычек нет, возможно текст указан явно
                # Пробуем найти после слов "текст" или "фразу"
                text_match = re.search(r'(?:текст|фразу)\s+([а-яА-Яa-zA-Z0-9\s!]+?)(?:\s+\d+|\s*$)',
            ...
            if 'сумм' in original_task:
                ...
                code += "    print('Вычисление не распознано')\n"
        elif task_type == 'char_pattern':
            code += "    print('AAA\\nAAA\\nAAA\\nAAA\\nAAA\\nAAA\\nBBBB\\nBBBB\\nBBBB\\nBBBB\\nBBBB\\nE\\nTTTTT\\nTTTTT\\nTTTTT\\nTTTTT\\nTTTTT\\nTTTTT\\nTTTTT\\nTTTTT\\nTTTTT\\nG')\n"
        elif task_type == 'repeat_sentence':
...
if __name__ == '__main__':
    ...
    # Дублированные методы (оставлены как в исходном коде, они не мешают)
    def _gen_fixed_loop_solution(self, count):
        return f"""def solve():
    for i in range({count}):
        print("Python is awesome!")
